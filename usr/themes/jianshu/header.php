...
<head <?php echo $screen_mode;?>>
    ...
    <?php endif; ?>
    <?php if (!empty($this->options->MColor) || !empty($this->options->HColor)): ?>
     <style type="text/css">a{color:<?php $this->options->MColor() ?>;}a:hover,a:active {color:<?php $this->options->HColor() ?>;}.btn.s3{border-color:<?php $this->options->MColor() ?>;background-color:<?php $this->options->MColor() ?>;}.btn.s3:hover{border-color:<?php $this->options->HColor() ?>;background-color:<?php $this->options->HColor() ?>;}.navbar-jianshu a:hover,.navbar-jianshu a:active,.navbar-jianshu a.active {background-color:<?php $this->options->MColor() ?>;text-shadow: 0 1px 0 <?php $this->options->MColor() ?>}.navbar-jianshu .dropdown a:hover {background-color:<?php $this->options->HColor() ?>}.navbar-jianshu .dropdown a:active {background-color:<?php $this->options->MColor() ?>}@media(max-width: 768px){.navbar-jianshu a{background-color:<?php $this->options->HColor() ?>;text-shadow: 0 1px 0 <?php $this->options->MColor() ?>;}}.search-form input:focus{border: 1px solid <?php $this->options->MColor() ?>;}.post-content a{color:<?php $this->options->MColor() ?>;}.comment-list li.comment-by-author {border-left:4px solid <?php $this->options->MColor() ?>;}#comment-form input:focus,#comment-form textarea:focus{border: 1px solid <?php $this->options->MColor() ?>;}.tag-list li a:hover{background-color:<?php $this->options->HColor() ?>;}.page-navigator>.current>a,.page-navigator>.current>a:focus,.page-navigator>.current>a:hover,.page-navigator>.current>span,.page-navigator>.current>span:focus,.page-navigator>.current>span:hover {border-color:<?php $this->options->MColor() ?>;background-color:<?php $this->options->MColor() ?>;}.ajax-navigator a {background-color:<?php $this->options->MColor() ?>;}.ajax-navigator a:hover{background:<?php $this->options->HColor() ?>;}.ajax-navigator li.current a{background-color:<?php $this->options->HColor() ?>;}.dialog.success{background-color:<?php $this->options->HColor() ?>;}body.night-mode .post-content a {color:<?php $this->options->MColor() ?>;}.night-mode .comment-list li.comment-by-author{border-left:4px solid <?php $this->options->MColor() ?>;}.night-mode .search-form input:focus,.night-mode #comment-form textarea:focus{border-color:<?php $this->options->MColor() ?>;}.night-mode .page-navigator li.current a{background-color:<?php $this->options->MColor() ?>;}.night-mode .ajax-navigator li.current a{background-color:<?php $this->options->MColor() ?>;}</style>
    <?php endif; ?>
    <?php if ($this->options->highlight == '1' ): ?>
...
<a class="logo dropdown-toggle<?php if($this->is('index')): ?> active<?php endif; ?>"  data-target="#nav-menu" href="<?php $this->options->siteUrl(); ?>"> <b> <?php if($this->options->logoText){$this->options->logoText();}else{echo mb_substr($this->options->title,0,1,'utf-8');} ?></b> <i class="fa fa-home"> </i> <span class="title"> <?php _e('首页'); ?> </span></a>
    ...
    <?php $this->widget('Widget_Contents_Page_List')->to($pages); ?>
        ...
        <?php endwhile; ?>
        <?php endif; ?>
    <?php if ($this->options->menu == 'cat'):  ?>
    <?php $this->widget('Widget_Metas_Category_List')->to($categories); ?>
        <?php while($categories->next()): ?>
        <a<?php if($this->is('category', $categories->slug)): ?> class="active"<?php endif; ?>  href="<?php $categories->permalink(); ?>"><?php $categories->name(); ?><i class="fa fa-<?php $categories->slug(); ?>"> </i></a>
        <?php endwhile; ?>
        <?php endif; ?> 
	</div> 
</div>
...
<div class="sidebar">
    ...
    <?php if (!empty($this->options->Show) && in_array('avatar', $this->options->Show)): ?>
