...
function themeConfig($form) {
    ...
    $Show = new Typecho_Widget_Helper_Form_Element_Checkbox('Show', array('avatar' => _t('首页头像'),'favatar' => _t('文章页头像'),'tags' => _t('文章页底部标签'),'author' => _t('文章页发布信息'),'tool' => _t('新版文章页工具集'),'old' => _t('旧版支付宝捐款'),'oldimg' => _t('旧版支付宝捐款的二维码'),'footer' => _t('全站底部分类及RSS订阅')),array('avatar', 'favatar',  'tags',  'footer' , 'tool' , 'author'), _t('自定义显示'));
    $form->addInput($Show->multiMode());
    $PageShow = new Typecho_Widget_Helper_Form_Element_Checkbox('PageShow', array('pfavatar' => _t('文章页头像'),'ptags' => _t('文章页底部标签'),'ptool' => _t('新版文章页工具集'),'pold' => _t('旧版支付宝捐款'),'poldimg' => _t('旧版支付宝捐款的二维码')),array('ptool'), _t('独立页面显示'));
    ...
    $MColor = new Typecho_Widget_Helper_Form_Element_Text('MColor', NULL, NULL, _t('自定义主色'), _t('请填写 16位颜色值(亮色)'),_t('<font color="#F40">如不使用请不要填写！</font>'));
    $form->addInput($MColor);
    $HColor = new Typecho_Widget_Helper_Form_Element_Text('HColor', NULL, NULL, _t('自定义配色'), _t('请填写16位颜色值(如:hover 暗色)'),_t('<font color="#F40">如不使用请不要填写！</font>'));
    $form->addInput($HColor);    
    $menu = new Typecho_Widget_Helper_Form_Element_Radio('menu', array('cat' => _t('分类目录'),'page' => _t('独立页面')),'page',_t('导航栏输出'));
    $form->addInput($menu);    
    $Navigator = new Typecho_Widget_Helper_Form_Element_Radio('Navigator',array('PageNav' => _t('默认风格'),'PageAjax' => _t('Ajax风格')),'PageNav',_t('底部分页'));
    $form->addInput($Navigator);
    $Pjax = new Typecho_Widget_Helper_Form_Element_Radio('Pjax',array('1' => _t('开启'),'0'=>_t('关闭')),'0',_t('Pjax全站防刷新'),_t("测试功能，尚未完善…"));
    $form->addInput($Pjax);
    $Indent = new Typecho_Widget_Helper_Form_Element_Radio('Indent',array('1'=>_t('开启'),'0'=>_t('关闭')),'0',_t("段落缩进"),_t("文章每个段落前自动缩进2个空格"));
    $form->addInput($Indent);     
    $lightbox = new Typecho_Widget_Helper_Form_Element_Radio('lightbox',array('1' => _t('开启'),'0'=>_t('关闭')),'1',_t('文章图片'),_t("开启后如原版仅显示图片名称描述，点击才显示图片"));
    $form->addInput($lightbox);
    $highlight = new Typecho_Widget_Helper_Form_Element_Radio('highlight',array('1' => _t('开启'),'0'=>_t('关闭')),'1',_t('代码高亮'),_t("开启后如原版高亮显示代码片段"));
    $form->addInput($highlight);    
    $post_author = new Typecho_Widget_Helper_Form_Element_Radio('post_author',array('1' => _t('图片地址'),'2' => _t('邮箱地址')),'2',_t('文章作者头像'),_t("图片地址使用博主头像</br>邮箱地址使用作者邮箱设置的gravatar头像"));
    ...
    $listStyle = new Typecho_Widget_Helper_Form_Element_Checkbox('listStyle',array('excerpt' => _t('显示摘要'),'thumb' => _t('显示缩略图')),array('excerpt', 'thumb'), _t('列表显示'));
